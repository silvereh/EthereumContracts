...
* 
* Note: ERC721Arr_Context_Mod_NoPrivFunc
*/
abstract contract ERC721Arr_110 is Context, IERC165, IERC721 {
	// Errors
	...
	*/
	modifier exists( uint256 tokenId_ ) {
		if ( tokenId_ >= _owners.length || _owners[ tokenId_ ] == address( 0 ) ) {
	...
	*/
	modifier isApprovedOrOwner( address operator_, uint256 tokenId_ ) {
		...
		bool _isApproved_ = _msgSender() == _tokenOwner_ ||
												_msgSender() == _tokenApprovals[ tokenId_ ] ||
												_operatorApprovals[ _tokenOwner_ ][ _msgSender() ];
		if ( ! _isApproved_ ) {
	...
	*/
	function _mint( address to_ ) internal virtual {
		uint256 _tokenId_ = _owners.length;
		_owners.push( to_ );
		emit Transfer( address( 0 ), to_, _tokenId_ );
	}
	...
	*/
	function approve( address to_, uint256 tokenId_ ) public virtual override exists( tokenId_ ) isApprovedOrOwner( _msgSender(), tokenId_ ) {
		address _tokenOwner_ = _owners[ tokenId_ ];
		if ( to_ == _tokenOwner_ ) {
	...
	*/
	function safeTransferFrom( address from_, address to_, uint256 tokenId_ ) public virtual override isValidTransfer( to_ ) exists( tokenId_ ) isApprovedOrOwner( _msgSender(), tokenId_ ) isSafeTransfer( to_, tokenId_, "" ) {
		address _tokenOwner_ = _owners[ tokenId_ ];
	...
	* but we cannot remove this parameter to stay in conformity with IERC721
	*/
	function safeTransferFrom( address from_, address to_, uint256 tokenId_, bytes memory data_ ) public virtual override isValidTransfer( to_ ) exists( tokenId_ ) isApprovedOrOwner( _msgSender(), tokenId_ ) isSafeTransfer( to_, tokenId_, data_ ) {
		address _tokenOwner_ = _owners[ tokenId_ ];
		_transfer( _tokenOwner_, to_, tokenId_ );
	}
	...
	* but we cannot remove this parameter to stay in conformity with IERC721
	*/
	function transferFrom( address from_, address to_, uint256 tokenId_ ) public virtual override isValidTransfer( to_ ) exists( tokenId_ ) isApprovedOrOwner( _msgSender(), tokenId_ ) {
		address _tokenOwner_ = _owners[ tokenId_ ];
		_transfer( _tokenOwner_, to_, tokenId_ );
	}
