...
const shouldBehaveLikeIWhitelisted = ( contract_name, contract_params ) => {
	describe( 'Should behave like IWhitelisted', () => {
				...
				describe( CONTRACT.METHODS.consumeWhitelist.SIGNATURE, () => {
					if ( TEST.METHODS.consumeWhitelist ) {
						it( 'Trying to consume whitelist while whitelist is not set should be reverted with ' + ERROR.IWhitelisted_FORBIDDEN, async () => {
							await expect( contract.consumeWhitelist( wl_user1_address, wl_user1_pass, wl_user1_flag, 1 ) ).to.be.revertedWith( ERROR.IWhitelisted_FORBIDDEN )
						})
					}
				})
				...
				describe( CONTRACT.METHODS.setWhitelist.SIGNATURE, () => {
					if ( TEST.METHODS.setWhitelist ) {
						it( 'Trying to set a whitelist with no allowance should be reverted with ' + ERROR.IWhitelisted_NO_ALLOWANCE, async () => {
							await expect( contract.setWhitelist( contract_params.PASS_ROOT, 0 ) ).to.be.revertedWith( ERROR.IWhitelisted_NO_ALLOWANCE )
						})
							...
							describe( CONTRACT.METHODS.consumeWhitelist.SIGNATURE, () => {
								if ( TEST.METHODS.consumeWhitelist ) {
									describe( 'Consuming 1 whitelist spot', () => {
										...
										it( 'Trying to consume an additional ' + contract_params.PASS_MAX + ' whitelist spots should be reverted with ' + ERROR.IWhitelisted_FORBIDDEN, async () => {
											await expect( contract.connect( wl_user1 ).consumeWhitelist( wl_user1_address, wl_user1_pass, wl_user1_flag, contract_params.PASS_MAX ) ).to.be.revertedWith( ERROR.IWhitelisted_FORBIDDEN )
										})
									})
...
module.exports = { shouldBehaveLikeIWhitelisted }
