...
const shouldBehaveLikeERC1155BaseMetadataURI = function( fixture, contract_params ) {
	describe( 'Should behave like ERC1155BaseMetadataURI', function() {
		...
		describe( 'Correct input ...', function() {
			if ( TEST.USE_CASES.CORRECT_INPUT ) {
				it( 'Contract should support IERC1155MetadataURI', async function() {
				...
				describe( CONTRACT.METHODS.uri.SIGNATURE, function() {
					if ( TEST.METHODS.uri ) {
						it( 'token URI should be an empty string', async function() {
							expect( await contract.uri( contract_params.INIT_SERIES ) ).to.equal( '' )
						})
					}
				})

				describe( CONTRACT.METHODS.setUri.SIGNATURE, function() {
					if ( TEST.METHODS.setUri ) {
						it( 'tokenURI should now be ' + contract_params.BASE_URI, async function() {
