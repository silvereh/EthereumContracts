...
const shouldBehaveLikeERC721Base = function( fixture, contract_params ) {
	describe( 'Should behave like ERC721Base', function() {
			...
			if ( TEST.USE_CASES.CORRECT_INPUT ) {
				...
				describe( CONTRACT.METHODS.balanceOf.SIGNATURE, function() {
					if ( TEST.METHODS.balanceOf ) {
						...
						it( 'Balance of null address should be 0', async function() {
							expect( await contract.balanceOf( CST.ADDRESS_ZERO ) ).to.equal( 0 )
						})
					}
				})
				...
				describe( CONTRACT.METHODS.getApproved.SIGNATURE, function() {
					if ( TEST.METHODS.getApproved ) {
						it( 'Approved addresses for unminted token should be reverted with "' + ERROR.IERC721_NONEXISTANT_TOKEN + '"', async function() {
				...
				describe( CONTRACT.METHODS.ownerOf.SIGNATURE, function() {
					if ( TEST.METHODS.ownerOf ) {
						it( 'Owner of unminted token should revert with ' + ERROR.IERC721_NONEXISTANT_TOKEN, async function() {
							...
							if ( true ) {
								...
								describe( CONTRACT.METHODS.approve.SIGNATURE, function() {
									if( TEST.METHODS.approve ) {
										it( 'Trying to approve transfer of a token not minted should be reverted with ' + ERROR.IERC721_NONEXISTANT_TOKEN, async function() {
										...
										it( 'Trying to approve transfer of a token not owned should be reverted with ' + ERROR.IERC721_CALLER_NOT_APPROVED, async function() {
										...
										describe( 'Approve transfer of token owned', function() {
											...
											describe( user1_name + ' trying to approve trading of token ' + contract_params.INIT_SUPPLY, function() {
												it( 'by ' + token_owner_name + ', should be reverted with ' + ERROR.IERC721_APPROVE_OWNER, async function() {
												...
												it( 'by someone else, should be be allowed and clear their approval', async function() {
													await contract.connect( user1 ).approve( user2_address, contract_params.INIT_SUPPLY )
								...
								describe( CONTRACT.METHODS.safeTransferFrom.SIGNATURE, function() {
									if( TEST.METHODS.safeTransferFrom ) {
										it( 'Trying to safe transfer a token not minted should be reverted with ' + ERROR.IERC721_NONEXISTANT_TOKEN, async function() {
										...
										it( 'Trying to safe transfer a token not owned should be reverted with ' + ERROR.IERC721_CALLER_NOT_APPROVED, async function() {
										...
										describe( 'Safe transfer of token ' + contract_params.INIT_SUPPLY + ' owned', function() {
											...
											describe( 'To the null address', function() {
												it( 'Should be reverted with ' + ERROR.IERC721_NULL_ADDRESS_TRANSFER, async function() {
											...
											describe( 'To other user', function() {
												...
												it( token_owner_name + ' trying to transfer token ' + contract_params.INIT_SUPPLY + ' back to them should be reverted with ' + ERROR.IERC721_CALLER_NOT_APPROVED, async function() {
												...
												it( token_owner_name + ' trying to safe transfer token ' + contract_params.INIT_SUPPLY + ' back to them should be reverted with ' + ERROR.IERC721_CALLER_NOT_APPROVED, async function() {
												...
												it( token_owner_name + ' trying to approve transfer of token ' + contract_params.INIT_SUPPLY + ' to them should be reverted with ' + ERROR.IERC721_CALLER_NOT_APPROVED, async function() {
								...
								describe( CONTRACT.METHODS.safeTransferFrom_ol.SIGNATURE, function() {
									if( TEST.METHODS.safeTransferFrom ) {
										it( 'Trying to safe transfer a token not minted should be reverted with ' + ERROR.IERC721_NONEXISTANT_TOKEN, async function() {
											await expect( contract.connect( token_owner ).functions[ CONTRACT.METHODS.safeTransferFrom_ol.SIGNATURE ]( token_owner_address, user2_address, contract_params.INIT_SUPPLY +  1, '0x' ) ).to.be.revertedWith( ERROR.IERC721_NONEXISTANT_TOKEN )
										})

										it( 'Trying to safe transfer a token not owned should be reverted with ' + ERROR.IERC721_CALLER_NOT_APPROVED, async function() {
											await expect( contract.connect( user1 ).functions[ CONTRACT.METHODS.safeTransferFrom_ol.SIGNATURE ]( contract_deployer_address, user1_address, contract_params.INIT_SUPPLY, '0x' ) ).to.be.revertedWith( ERROR.IERC721_CALLER_NOT_APPROVED )
										})
										...
										describe( 'Safe transfer of token ' + contract_params.INIT_SUPPLY + ' owned', function() {
											...
											describe( 'To the null address', function() {
												it( 'Should be reverted with ' + ERROR.IERC721_NULL_ADDRESS_TRANSFER, async function() {
													await expect( contract.connect( token_owner ).functions[ CONTRACT.METHODS.safeTransferFrom_ol.SIGNATURE ]( token_owner_address, CST.ADDRESS_ZERO, contract_params.INIT_SUPPLY, '0x'  ) ).to.be.revertedWith( ERROR.IERC721_NULL_ADDRESS_TRANSFER )
												})
											})
											...
											describe( 'To other user', function() {
												...
												it( token_owner_name + ' trying to safe transfer token ' + contract_params.INIT_SUPPLY + ' back to them should be reverted with ' + ERROR.IERC721_CALLER_NOT_APPROVED, async function() {
													await expect( contract.connect( token_owner ).functions[ CONTRACT.METHODS.safeTransferFrom_ol.SIGNATURE ]( user1_address, token_owner_address, contract_params.INIT_SUPPLY, '0x' ) ).to.be.revertedWith( ERROR.IERC721_CALLER_NOT_APPROVED )
												})
											...
											describe( 'To non ERC721Receiver contract', function() {
												it( 'Should be reverted with ' + ERROR.IERC721_NON_ERC721_RECEIVER, async function() {
													await expect( contract.connect( token_owner ).functions[ CONTRACT.METHODS.safeTransferFrom_ol.SIGNATURE ]( token_owner_address, contract_address, contract_params.INIT_SUPPLY, '0x' ) ).to.be.revertedWith( ERROR.IERC721_NON_ERC721_RECEIVER )
												})
											})
											...
											describe( 'To a receiver contract returning unexpected value', function () {
												it( 'Should be reverted with ' + ERROR.IERC721_NON_ERC721_RECEIVER, async function () {
													const invalidReceiver = await holder_artifact.deploy( CST.INTERFACE_ID.IERC165, Error.None )
													await expect( contract.connect( token_owner ).functions[ CONTRACT.METHODS.safeTransferFrom_ol.SIGNATURE ]( token_owner_address, invalidReceiver.address, contract_params.INIT_SUPPLY, '0x'  ) ).to.be.revertedWith( ERROR.IERC721_NON_ERC721_RECEIVER )
												})
											})
								...
								describe( CONTRACT.METHODS.transferFrom.SIGNATURE, function() {
									if( TEST.METHODS.transferFrom ) {
										it( 'Trying to transfer a token not minted should be reverted with ' + ERROR.IERC721_NONEXISTANT_TOKEN, async function() {
										...
										it( 'Trying to transfer a token not owned should be reverted with ' + ERROR.IERC721_CALLER_NOT_APPROVED, async function() {
