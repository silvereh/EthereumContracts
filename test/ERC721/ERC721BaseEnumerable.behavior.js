			...
			if ( TEST.USE_CASES.CORRECT_INPUT ) {
				...
				describe( CONTRACT.METHODS.tokenOfOwnerByIndex.SIGNATURE, () => {
					if ( TEST.METHODS.tokenOfOwnerByIndex ) {
						it( 'Trying to get unminted token index should be reverted with ' + ERROR.IERC721Enumerable_NONEXISTANT_TOKEN, async () => {
							await expect( contract.tokenByIndex( contract_params.INIT_SUPPLY + 1 ) ).to.be.revertedWith( ERROR.IERC721Enumerable_NONEXISTANT_TOKEN )
						})
				...
				describe( CONTRACT.METHODS.tokenByIndex.SIGNATURE, () => {
					if ( TEST.METHODS.tokenByIndex ) {
						it( 'Trying to get unminted token index should be reverted with ' + ERROR.IERC721Enumerable_NONEXISTANT_TOKEN, async () => {
							await expect( contract.tokenOfOwnerByIndex( token_owner_address, contract_params.INIT_SUPPLY + 1 ) ).to.be.revertedWith( ERROR.IERC721Enumerable_NONEXISTANT_TOKEN )
						})
						...
						describe( 'After minting a token into ' + token_owner_name + '\'s wallet', () => {
							...
							it( 'Trying to get token of the null address should be reverted with ' + ERROR.IERC721Enumerable_NULL_ADDRESS_BALANCE, async () => {
								await expect( contract.tokenOfOwnerByIndex( CST.ADDRESS_ZERO, contract_params.INIT_SUPPLY ) ).to.be.revertedWith( ERROR.IERC721Enumerable_NULL_ADDRESS_BALANCE )
							})
							...
							describe( 'After minting another token into ' + token_owner_name + '\'s wallet', () => {
								...
								describe( 'After minting a token into ' + user1_name + '\'s wallet', () => {
									beforeEach( async () => {
										await contract.mint( user1_address )
									})
