...
const shouldBehaveLikeERC721OZ = ( contract_name, contract_params ) => {
	describe( 'Should behave like ERC721OZ', () => {
			...
			if ( TEST.USE_CASES.CORRECT_INPUT ) {
				...
				describe( CONTRACT.METHODS.balanceOf.SIGNATURE, () => {
					if ( TEST.METHODS.balanceOf ) {
						...
						it( 'Balance of null address should be reverted with "' + ERROR.OZERC721_NULL_ADDRESS_BALANCE + '"', async () => {
				...
				describe( CONTRACT.METHODS.getApproved.SIGNATURE, () => {
					if ( TEST.METHODS.getApproved ) {
						it( 'Approved addresses for unminted token should be reverted with "' + ERROR.OZERC721_NONEXISTANT_TOKEN + '"', async () => {
				...
				describe( CONTRACT.METHODS.ownerOf.SIGNATURE, () => {
					if ( TEST.METHODS.ownerOf ) {
						it( 'Owner of unminted token should revert with ' + ERROR.OZERC721_NONEXISTANT_TOKEN, async () => {
							...
							if ( true ) {
								...
								describe( CONTRACT.METHODS.approve.SIGNATURE, () => {
									if( TEST.METHODS.approve ) {
										it( 'Trying to approve transfer of a token not minted should be reverted with ' + ERROR.OZERC721_NONEXISTANT_TOKEN, async () => {
										...
										it( 'Trying to approve transfer of a token not owned should be reverted with ' + ERROR.OZERC721_CALLER_NOT_APPROVED_APPROVE, async () => {
										...
										describe( 'Approve transfer of token owned', () => {
											...
											describe( user1_name + ' trying to approve trading of token ' + contract_params.INIT_SUPPLY, () => {
												it( 'by ' + token_owner_name + ', should be reverted with ' + ERROR.OZERC721_APPROVE_OWNER, async () => {
												...
												it( 'by someone else, should be reverted with "' + ERROR.OZERC721_CALLER_NOT_APPROVED_APPROVE + '"', async () => {
								...
								describe( CONTRACT.METHODS.safeTransferFrom.SIGNATURE, () => {
									if( TEST.METHODS.safeTransferFrom ) {
										it( 'Trying to safe transfer a token not minted should be reverted with ' + ERROR.OZERC721_NONEXISTANT_TOKEN, async () => {
										...
										it( 'Trying to safe transfer a token not owned should be reverted with ' + ERROR.OZERC721_CALLER_NOT_APPROVED_TRANSFER, async () => {
										...
										describe( 'Safe transfer of token ' + contract_params.INIT_SUPPLY + ' owned', () => {
											...
											describe( 'To the null address', () => {
												it( 'Should be reverted with ' + ERROR.OZERC721_NULL_ADDRESS_TRANSFER, async () => {
											...
											describe( 'To other user', () => {
												...
												it( token_owner_name + ' trying to transfer token ' + contract_params.INIT_SUPPLY + ' back to them should be reverted with ' + ERROR.OZERC721_CALLER_NOT_APPROVED_TRANSFER, async () => {
												...
												it( token_owner_name + ' trying to safe transfer token ' + contract_params.INIT_SUPPLY + ' back to them should be reverted with ' + ERROR.OZERC721_CALLER_NOT_APPROVED_TRANSFER, async () => {
												...
												it( token_owner_name + ' trying to approve transfer of token ' + contract_params.INIT_SUPPLY + ' to them should be reverted with ' + ERROR.OZERC721_CALLER_NOT_APPROVED_APPROVE, async () => {
											...
											describe( 'To non ERC721Receiver contract', () => {
												it( 'Should be reverted with ' + ERROR.OZERC721_NON_ERC721_RECEIVER, async () => {
								...
								describe( CONTRACT.METHODS.safeTransferFrom_ol.SIGNATURE, () => {
									if( TEST.METHODS.safeTransferFrom ) {
										it( 'Trying to safe transfer a token not minted should be reverted with ' + ERROR.OZERC721_NONEXISTANT_TOKEN, async () => {
								...
								describe( CONTRACT.METHODS.setApprovalForAll.SIGNATURE, () => {
									if( TEST.METHODS.setApprovalForAll ) {
										...
										it( 'Trying to allow self should be reverted with ' + ERROR.OZERC721_APPROVE_CALLER, async () => {
								...
								describe( CONTRACT.METHODS.transferFrom.SIGNATURE, () => {
									if( TEST.METHODS.transferFrom ) {
										it( 'Trying to transfer a token not minted should be reverted with ' + ERROR.OZERC721_NONEXISTANT_TOKEN, async () => {
										...
										it( 'Trying to transfer a token not owned should be reverted with ' + ERROR.OZERC721_CALLER_NOT_APPROVED_TRANSFER, async () => {
